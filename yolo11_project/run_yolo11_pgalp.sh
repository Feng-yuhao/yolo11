#!/bin/bash
# E9b formal 200-epoch launcher. Automatic shutdown is disabled everywhere.
set -u
if [ "$#" -ne 0 ]; then
...
SCRIPT="$PROJECT/yolo11s_p2_pgalp_img1024_seed1.py"
export OMP_NUM_THREADS=8
cd "$PROJECT" || exit 1
for REQUIRED in \
    "$SCRIPT" \
    "$PROJECT/yolo11s-p2-pgalp.yaml" \
...
if [ ! -f "$PROJECT/comparison_reports/e9b_pgalp_tal2_smoke_passed.json" ]; then
    echo "Run E9b --smoke2 first. Formal training was not started."
    exit 1
fi
mkdir -p "$PROJECT/logs" || exit 1
LOG="$PROJECT/logs/e9b_pgalp_formal_$(date +%Y%m%d_%H%M%S)_$$.log"
