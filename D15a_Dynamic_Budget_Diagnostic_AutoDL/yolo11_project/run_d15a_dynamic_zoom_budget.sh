...
export OMP_NUM_THREADS="${D15A_OMP_NUM_THREADS:-8}"
export MKL_NUM_THREADS="${D15A_MKL_NUM_THREADS:-8}"
export PYTHONPATH="/root/autodl-tmp/yolo11_src${PYTHONPATH:+:${PYTHONPATH}}"

stamp="$(date +%Y%m%d_%H%M%S)"
log="/root/autodl-tmp/yolo11_project/logs/d15a_dynamic_zoom_budget_${stamp}_$$.log"
echo "D15a diagnostic log: ${log}"
echo "This run performs no training and never shuts down the instance."

set +e
/root/miniconda3/envs/yolo11/bin/python -u \
  diagnose_d15a_dynamic_zoom_budget.py "$@" 2>&1 | tee "${log}"
status=${PIPESTATUS[0]}
set -e
