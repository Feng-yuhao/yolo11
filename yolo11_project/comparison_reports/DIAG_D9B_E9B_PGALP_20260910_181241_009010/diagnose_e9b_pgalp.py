#!/usr/bin/env python3
"""D9b: zero-training causal and routing audit for the frozen E9b PGALP model.
...
WEIGHTS = PROJECT / "runs/e9b_yolo11s_p2_pgalp_img1024_seed1/weights/best.pt"
E9B_REPORT = (
    PROJECT
    / "comparison_reports/e9b_yolo11s_p2_pgalp_img1024_seed1_20260910_021529_053852/metrics.json"
)
CORE_SCRIPT = PROJECT / "yolo11s_p2_img1024_seed1.py"
PGALP_SOURCE = SOURCE / "ultralytics/nn/modules/pgalp.py"
...
MAX_DET = 500
HIST_BINS = 200
SCRIPT_REVISION = "d9b_pgalp_causal_routing_audit_v1"
...
def find_pgalp(network):
    found = [module for module in network.modules() if module.__class__.__name__ == "PGALP"]
    if len(found) != 1:
        raise RuntimeError(f"Expected exactly one PGALP module, found {len(found)}")
    module = found[0]
    if tuple(module.kernels) != (3, 5, 7) or tuple(module.sigmas) != (0.8, 1.2, 1.8):
        raise RuntimeError("Checkpoint PGALP filter bank differs from E9b")
    if module.p2_channels != 128 or module.p3_channels != 128:
        raise RuntimeError("Checkpoint PGALP channels differ from E9b")
    return module
...
def write_text_report(report, path):
    lines = [
        "D9b PGALP zero-training diagnostic",
...
def main():
    ...
    report = {
        "status": "starting",
        "purpose": "D9b zero-training causal and routing audit of frozen E9b PGALP",
    ...
    try:
        ...
        if E9B_REPORT.is_file() and "normal" in mode_metrics and not args.smoke_images:
            ...
            report["normal_reproduction_vs_frozen_e9b"] = {
